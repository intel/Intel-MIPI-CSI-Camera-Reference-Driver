...
for camera in $mux_list; do
  ...
  for sens in "${!d4xx_vc_named[@]}"; do
    ...
    [[ -z $vid ]] && continue;
    dev_entity=$(echo "${dot}" | grep "${vid_nd}" | grep label | awk -F'"' '{print $2}' | awk -F'/dev' '{print $1}' | awk -F'\' '{print $1}')
    csi2_pad=$(${media_util} -d ${mdev} -e "${dev_entity}" -p | awk -F'<-' '{print $2}'  | awk -F'[' '{print $1}' )
    csi2_entity=$(echo $csi2_pad | awk -F':' '{print $1}' | tr -d '"')
    csi2_stream=$(($(echo $csi2_pad | awk -F':' '{print $2}')-1))
    dev_ln="/dev/video-rs-${camera_names["${sens}"]}-${suffix}"
    dev_name=$(${v4l2_util} -d $vid -D | grep "Card type" | awk -F':' '{print $2}')
    is_route_disabled=$(${media_util} -d ${mdev} -e "${csi2_entity}" -p | grep "0/${csi2_stream} ->" | grep INACTIVE)
    if [[ -z ${is_route_disabled} ]]; then
	link_state="Stream(id=$csi2_stream)"
    else
	link_state="Inactive"
    fi
