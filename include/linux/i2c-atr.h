 ...
 *                 alias.
 * @select: Optional callback before a transfer on a child bus.
 * @deselect: Optional callback after a transfer on a child bus.
 *
...
struct i2c_atr_ops {
	...
	void (*deselect)(struct i2c_atr *atr, u32 chan_id);
};
...
/**
 * i2c_atr_get_adapter - Return a child adapter added to an ATR helper.
 * @atr:     The I2C ATR
 * @chan_id: Index of the adapter to return
 *
 * Return: The child adapter for @chan_id, or NULL if it has not been added.
 */
struct i2c_adapter *i2c_atr_get_adapter(struct i2c_atr *atr, u32 chan_id);
