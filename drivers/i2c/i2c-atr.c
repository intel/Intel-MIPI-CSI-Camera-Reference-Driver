...
static int i2c_atr_parse_alias_pool(struct i2c_atr *atr)
{
	...
	if (ret < 0) {
		dev_dbg(dev, "No i2c-alias-pool property; using identity aliases with passthrough\n");
		return 0;
	}
...
{
	if (!atr || chan_id >= atr->max_adapters)
		return NULL;

	return atr->adapter[chan_id];
}
EXPORT_SYMBOL_NS_GPL(i2c_atr_get_adapter, I2C_ATR);
