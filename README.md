...
export $HOME=$(pwd)
cd $HOME
git clone --recursive https://github.com/Pirouf/Intel-mipi-gmsl-modules.git -b 20260421/Q1.2-d4xx-max9672x intel-mipi-gmsl-drivers-debian-20260421
```

2. Create origin debian source archive

```
tar czvf intel-mipi-gmsl-drivers_20260421.orig.tar.gz --exclude=".git" --exclude=".gitignore" --exclude=".gitmodules" --exclude="debian"  --exclude=".pc" intel-mipi-gmsl-drivers-debian-20260421
```
...
cd $HOME/intel-mipi-gmsl-drivers-debian-20260421
...
```
cd $HOME
sudo apt install ./intel-mipi-gmsl-dkms_20260421-1_amd64.deb ./v4l-utils_1.32.0-2_amd64.deb ./libv4l*_1.32.0-2_amd64.deb
```
...
Configure each Camera ACPI device individualy `Intel Advanced Menu -> System Agent (SA) Configuration -> MIPI Camera Configuration -> Link Options`.
...
| I2C Channel                              | I2C1       | I2C1      | I2C1       | I2C1       |
...
# PTL GMSL2 AIC dual-max96724 device generally on i2c-0 and i2c-1 buses (with VC_EXTENSION enabled)
#options d4xx des_addr=0x27,0x27,0x27,0x27 serdes_bus=0,0,1,1 des_csi=0,0,2,2 sensor_vc=0,1,2,3,2,3,0,1,5,4,7,6,7,6,5,4
